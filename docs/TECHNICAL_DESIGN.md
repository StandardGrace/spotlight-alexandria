...
- **Nav bar added once the restaurants list page crossed the "second page" threshold** named in the layout strategy below. Rendered once via a shared per-language layout (nested routes + `<Outlet/>` in `App.jsx`) rather than duplicated per page. The language switcher, previously drawn inline on the one page that existed, moved into it.
- **Language switcher preserves the current path when switching languages.** It originally jumped straight to `/en` or `/fr` regardless of what page you were on — harmless with a single page, but would have silently broken (bouncing back to home) the first time someone switched languages from a page other than the homepage. Fixed as part of the nav bar work.
- **Bilingual restaurant fields are resolved client-side**, via a small `pick(bilingual, lang)` helper keyed off `i18n.language` — the same backend-returns-both-languages/frontend-picks pattern already used for weather, now extended to restaurant content.
...
- **Navigation infrastructure is added only once it's needed** — i.e. only once a second page actually exists — rather than pre-built for a hypothetical future sitemap. (That threshold was crossed when the restaurants list page was built — see the nav bar entry in the frontend decision log above.)
...
- **`lastVerified` date field, with staleness surfaced in the UI.** If a restaurant's info hasn't been reverified in over 6 months, the UI shows a "call ahead to confirm" notice rather than presenting stale information as current. Tone decided: softened, visitor-framed copy rather than an alarming warning. The restaurant detail page now exists, but this notice hasn't been added to it yet — still the one unbuilt piece of this content model.
- **Storefront photo and menu-item photo are handled as separate fields, and both are optional.** The storefront photo is self-shot when present, and a restaurant can be entered and ingested before anyone's had a chance to visit and take one; menu-item photos are optional too, and only included when restaurant-provided. Kept as distinct fields so licensing/provenance isn't conflated between the two even though neither is required.
- **Grid landing view → detail page per restaurant. Built.** Restaurants list page (`/:lang/restaurants`) and restaurant detail page (`/:lang/restaurants/:id`), both reading from `main-site-api`'s restaurants route. Multi-variant menu items (e.g., different sizes) get an inline size/price `<select>` on the detail page rather than being listed as separate line items.

#### Presentation layer

Implemented in `RestaurantsListPage.jsx` and `RestaurantDetailPage.jsx`:
...
- **Multi-variant items use an inline selector, not a third accordion level.** Keeps disclosure depth at two (restaurant page → category), consistent with the same UX guidance above. Built as a native `<select>` per item — there's no ordering/cart system on the site, so this exists purely to keep the menu compact, not as a step toward one.
- **Accordions built with native `<details>`/`<summary>`, not custom JS state.** Free keyboard and screen-reader accessibility, and matches the frontend's existing preference for minimal, easy-to-read code with no state management library. Categories are collapsed by default.
- **Missing storefront/item photos get a plain placeholder, not a "coming soon" treatment.** A restaurant with no storefront photo shows a tinted box with its first initial, on both the list and detail pages. A menu item with no photo simply omits the image rather than showing a placeholder — an item photo was always meant to be a nice-to-have, not an expected element. This reflects that the photo fields are permanently optional (see the content-model decision above), not temporarily incomplete.

### Known gaps (discovered 2026-08-27, not yet ticketed)

- **No site-wide 404/error page.** `App.jsx` has no catch-all route. A bad restaurant id under `/:lang/restaurants/:id` shows a real error state (the detail page's own error handling), but any other unmatched path renders a blank content area under the nav bar rather than a proper "page not found" state or redirect.
- **Colors are hardcoded throughout `index.css`** rather than defined once and referenced — the same handful of hex values (`#4a4f4c`, `#2a6f6f`, `#dee1df`, etc.) are repeated across many rules. Fine at this scale today — everything's in one file with semantic, component-scoped class names — but a future palette change would mean a careful find-and-replace rather than a quick edit. Worth a one-time refactor into CSS custom properties (`--color-*`) as the *first* step of whatever polish pass eventually touches the visual direction; that same refactor also makes **dark mode** straightforward afterward (override the variable values under `prefers-color-scheme`/a class, instead of re-touching every rule).

## Roadmap

### Next up

1. **Restaurant Menu Data Model & Ingestion** — data model and ingestion pipeline for restaurant listings and menus (see "Restaurant menu content model" above, and [`docs/restaurant-data-model.md`](docs/restaurant-data-model.md) for the full field spec), the last piece of the "first iteration" scope named in the README. **In progress — 4 of 5 checklist items functionally done:** the content format, the parser/SQLite ingestion pipeline, storefront/menu-item photo handling (including the frontend's missing-photo treatment), and the grid landing + detail pages are all built and working. Only the `lastVerified` staleness notice remains unbuilt, plus a final close-out validation pass (ingesting and rendering multiple real restaurants end-to-end) before the checklist can be marked fully complete.
